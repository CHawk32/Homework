...
int (*functionPtr[10])(char pathname[]);
...
int cd_func(char pathname[])
{
    ...
    if (success < 0) {
        printf("CD: Pathname could not be found.\n");
        return 0;
    }
    return 1;
}
...
int rmdir_func(char pathname[])
{
    ...
    if (success < 0) {
        printf("Error in removing directory %s.\n", pathname);
        return 0;
    }
    return 1;
}
...
int cat_func(char pathname[])
{
    ...
    if (fd <= 0) {
        printf("File %s could not be open for read mode.\n", pathname);
        return 0;
    } else {
