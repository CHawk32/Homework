...
main (int argc, char *argv[ ]) {
    ...
    char tmp = 0;
    char line[MAX], output[MAX], command[16], pathname[256], outputString[512];
    char result[256] = {0};
    ...
    while (1) {
        ...
        if (line[0] != 0) // exit if NULL line
        {
            ...
            remoteFunctionNum = checkClientCommand(command, pathname, 1);
            outputString[0] = 0;
            ...
            if (remoteFunctionNum > -1) {
                ...
                if(remoteFunctionNum != 7 && remoteFunctionNum != 8 ) // if get or put their functions control server connecitons
                {
                    // Wait for response from server
                    n = read(sock, returnString, 4096);
                    printf("\tServer (%d bytes):\n\n%s\n\tEnd of server Transmission.\n", n, returnString);
                }
